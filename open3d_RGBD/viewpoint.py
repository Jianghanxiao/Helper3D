...
if __name__ == "__main__":
    sum = []
    for i in range(5):
        color_img = f'{DATA_PATH}origin/{model}-{index}{i}.png'
        depth_img = f'{DATA_PATH}depth/{model}-{index}{i}_d.png'
        # Read the annotation
        annotation_file = open(
            f'{DATA_PATH}origin_annotation/{model}-{index}{i}.json')
        ...
        sum += [pcd, world] + camera + motions
    # Final Visualization
    o3d.visualization.draw_geometries(sum)
